...
function deleteReg(ruta, model, id){
//	alert(ruta + " " + model + " " + id); return false;
	$("#content").prepend('<div id="alertaeliminar"><div class="divAlerta"></div></div>');
	$("#alertaeliminar div.divAlerta").html('Desea eliminar el registro?');
	$( "#alertaeliminar" ).dialog({
      buttons: {
			...
			Confirmar: function() {
				...
				$.post(ruta,{
					...
					if(data.error && data.error != ''){
						$("#alertaeliminar div.divAlerta").html(data.error);
						$( "#alertaeliminar" ).dialog({
						      buttons: {
						    	  Cerrar: function() {
										$( this ).dialog( "close" );
						...
						$("#alertaeliminar").dialog("open"); return false;
					}
					if(data.mensaje && data.mensaje != ''){
						$("#alertaeliminar div.divAlerta").html(data.mensaje);
						$( "#alertaeliminar" ).dialog({
						      buttons: {
						    	  Aceptar: function() {
										$( this ).dialog( "close" );
...
// Refrescar el valor de un combo
$.fn.selectRefresh = function() {
	$(this).parent('.select').children('.select-value').text($('#' + $(this).attr('id') + ' option:selected').text());
};

// Refrescar el valor de un switch
$.fn.switchRefresh = function() {
	$(this).parent('switch').addClass('checked');
};